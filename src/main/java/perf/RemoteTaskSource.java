...
class RemoteTaskSource extends Thread implements TaskSource {
    ...
    newClient: while (true) {
      ...
      try {
        ...
        while (true) {
          ...
          task.queSize = queue.size();
          queue.put(task);
          // System.out.println("S: add " + s + "; size=" + queue.size() + "id=" + task.taskID);
        }
      } catch (Exception e) {
  ...
  @Override
  public void taskDone(Task task) throws IOException {
    // System.out.println("taskDone task:" + task.taskID + " totalHitCount:" + task.totalHits.value);
    if (out != null) {
      try {
        // NOTE: can cause NPE here (we are not sync'd)
        // but caller will print & ignore it...
        out.write(String
            .format(Locale.ENGLISH, "%8d:%9d:%16d:%16d:%16d:%16d:%16d:%8d:%8d", task.taskID, task.totalHits.value,
                task.recvTimeNS, task.processTimeNS, task.finishTimeNS, task.instructions, task.cycles, task.queSize, task.cpuId)
            .getBytes("UTF-8"));
