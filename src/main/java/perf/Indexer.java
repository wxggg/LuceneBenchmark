...
public final class Indexer {
  ...
  private static MergeScheduler getMergeScheduler(final AtomicBoolean indexingFailed, boolean useCMS,
    ...
    if (useCMS) {
      ...
      // Gives better repeatability because if you use CMS, the order in which the
      // merges complete can impact how the merge policy later
      // picks merges so you can easily get a very different index structure when you
      // are comparing two indices:
  ...
  private static void _main(String[] clArgs) throws Exception {
    ...
    // EG: -facets taxonomy;Date -facets taxonomy;Month -facets
    // sortedset:facetGroupField;Month
    ...
    final Codec codec = new Lucene80Codec() {
      ...
      // DocValuesFormat.forName("Lucene42");
      // private final DocValuesFormat diskDVFormat = DocValuesFormat.forName("Disk");
      // private final DocValuesFormat lucene45DVFormat =
      ...
      public DocValuesFormat getDocValuesFormatForField(String field) {
        if (facetFields.contains(field)) {
          ...
          // System.out.println("DV: field=" + field + " format=" +
          // super.getDocValuesFormatForField(field));
    ...
    // Very tricky: if the line file docs source is binary, and you use multiple
    // threads, and you use grouping fields, then the doc count
    // may not match:
    ...
    if (countShouldMatch && w.getDocStats().maxDoc != docCountLimit) {
      throw new RuntimeException("w.maxDoc()=" + w.getDocStats().maxDoc + " but expected " + docCountLimit + " (off by "
