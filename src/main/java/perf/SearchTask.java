...
final class SearchTask extends Task {
  ...
  public SearchTask(String category, Query q, Sort s, String group, int topN, boolean doHilite, boolean doStoredLoads,
      List<String> facetRequests, boolean doDrillSideways) {
  ...
  public void go(IndexState state) throws IOException {
    // System.out.println("go group=" + this.group + " single=" + singlePassGroup +
    // " xxx=" + xxx + " this=" + this);
      ...
      if (group != null) {
        if (singlePassGroup) {
          final BlockGroupingCollector c = new BlockGroupingCollector(Sort.RELEVANCE, 10, true,
              searcher.createWeight(searcher.rewrite(state.groupEndQuery), ScoreMode.COMPLETE_NO_SCORES, 1));
          searcher.search(q, c);
          ...
          final FirstPassGroupingCollector<BytesRef> c1 = new FirstPassGroupingCollector(new TermGroupSelector(group),
              Sort.RELEVANCE, 10);
          ...
          if (topGroups != null) {
            final TopGroupsCollector<BytesRef> c2 = new TopGroupsCollector<>(new TermGroupSelector(group), topGroups,
                Sort.RELEVANCE, Sort.RELEVANCE, 10, true);
            searcher.search(q, c2);
            groupsResultTerms = c2.getTopGroups(0);
            if (allGroupsCollector != null) {
              groupsResultTerms = new TopGroups<BytesRef>(groupsResultTerms, allGroupsCollector.getGroupCount());
            }
            if (doHilite) {
        ...
        if (doDrillSideways) {
          ...
          for (String request : facetRequests) {
            if (request.startsWith("range:")) {
              ...
              // TODO: fixme to handle N facets in one indexed field! Need to make the facet
              // counts once per indexed field...
              Facets facets = new FastTaxonomyFacetCounts(state.facetsConfig.getDimConfig(request).indexFieldName,
              ...
              // TODO: fixme to handle N facets in one SSDV field! Need to make the facet
              // counts once per indexed field...
              SortedSetDocValuesReaderState ssdvFacetsState = state
                  .getSortedSetReaderState(state.facetsConfig.getDimConfig(request).indexFieldName);
            ...
            if (request.startsWith("range:")) {
              ...
              if (i == -1) {
                throw new IllegalArgumentException(
                    "range facets request \"" + request + "\" is missing field; should be range:field:0-10,10-20");
              }
        ...
        /*
         * final boolean fillFields = true; final boolean fieldSortDoTrackScores = true;
         * final boolean fieldSortDoMaxScore = true; final TopFieldCollector c =
         * TopFieldCollector.create(s, topN, fillFields, fieldSortDoTrackScores,
         * fieldSortDoMaxScore, false); searcher.search(q, c); hits = c.topDocs();
         */
      }
      if (hits != null) {
        ...
        totalHits = new TotalHits(groupsResultBlock.totalHitCount, TotalHits.Relation.EQUAL_TO);
      }
    } catch (Throwable t) {
  ...
  private void hilite(TopDocs hits, IndexState indexState, IndexSearcher searcher, Query query) throws IOException {
    ...
    if (indexState.fastHighlighter != null || indexState.useHighlighter) {
      ...
      /*
       * String[] frags =
       * indexState.postingsHighlighter.highlight(indexState.textFieldName, query,
       * searcher, hits, 2); //System.out.println("  q=" + query + ": hilite time: " +
       * ((t1-t0)/1000000.0)); for(int hit=0;hit<frags.length;hit++) { String frag =
       * frags[hit]; //System.out.println("  title=" +
       * searcher.doc(hits.scoreDocs[hit].doc).get("titleTokenized"));
       * //System.out.println("    frags: " + frag); if (frag != null) { // It's fine
       * for frag to be null: it's a // placeholder, meaning this hit had no hilite
  ...
  private void hilite(int docID, IndexState indexState, IndexSearcher searcher) throws IOException {
    ...
    if (indexState.fastHighlighter != null) {
      for (String h : indexState.fastHighlighter.getBestFragments(fieldQuery, searcher.getIndexReader(), docID,
          indexState.textFieldName, 100, 2)) {
    ...
    return "cat=" + category + " q=" + q + " s=" + s + " group=" + (group == null ? null : group.replace("\n", "\\n"))
        + (group == null
            ...
            : " groups=" + (singlePassGroup
                ? (groupsResultBlock.groups.length + " hits=" + groupsResultBlock.totalHitCount + " groupTotHits="
                    + groupsResultBlock.totalGroupedHitCount + " totGroupCount=" + groupsResultBlock.totalGroupCount)
                : (groupsResultTerms.groups.length + " hits=" + groupsResultTerms.totalHitCount + " groupTotHits="
                    + groupsResultTerms.totalGroupedHitCount + " totGroupCount=" + groupsResultTerms.totalGroupCount)))
    ...
    try {
      if (group != null) {
        if (singlePassGroup) {
          ...
          for (GroupDocs<BytesRef> groupDocs : groupsResultTerms.groups) {
            out.println("  group="
                + (groupDocs.groupValue == null ? "null" : groupDocs.groupValue.utf8ToString().replace("\n", "\\n"))
        ...
        for (int idx = 0; idx < hits.scoreDocs.length; idx++) {
          ...
          out.println("  doc=" + LineFileDocs.idToInt(searcher.doc(hit.doc).get("id")) + " " + s.getSort()[0].getField()
              + "=" + vs);
        }
      } else if (hits != null) {
        for (ScoreDoc hit : hits.scoreDocs) {
