...
public class SearchPerfTest {
  ...
  private static void _main(String[] clArgs) throws Exception {
     ...
     * facetsDir = new NativePosixMMapDirectory(new File(facetsDirPath)); } } else
     * if (dirImpl.equals("CachingDirWrapper")) { dir0 = new CachingRAMDirectory(new
     * MMapDirectory(new File(dirPath))); ramDir = null; } else if
     * (dirImpl.equals("RAMExceptDirectPostingsDirectory")) { // Load only
     * non-postings files into RAMDir (assumes // Lucene40PF is the wrapped PF):
     * Set<String> postingsExtensions = new HashSet<String>();
     * postingsExtensions.add("frq"); postingsExtensions.add("prx");
     * postingsExtensions.add("tip"); postingsExtensions.add("tim");
     * 
     * ramDir = new RAMDirectory(); Directory fsDir = new MMapDirectory(new
     * File(dirPath)); for (String file : fsDir.listAll()) { int idx =
     * file.indexOf('.'); if (idx != -1 &&
     * postingsExtensions.contains(file.substring(idx+1, file.length()))) {
     * continue; }
     * 
     * fsDir.copy(ramDir, file, file, IOContext.READ); } dir0 = new
     * FileSwitchDirectory(postingsExtensions, fsDir, ramDir, true); if (doFacets) {
    ...
    // now reflect
    final Class<? extends Similarity> simClazz = Class.forName("org.apache.lucene.search.similarities." + similarity)
    ...
    if (args.getFlag("-nrt")) {
      ...
      iwc.setMergedSegmentWarmer(new IndexWriter.IndexReaderWarmer() {
        ...
        public void warm(LeafReader reader) throws IOException {
          ...
          System.out
              .println("warm segment=" + reader + " numDocs=" + reader.numDocs() + ": took " + (t1 - t0) + " msec");
        }
      });
      ...
      LineFileDocs lineFileDocs = new LineFileDocs(lineDocsFile, false, storeBody, tvsBody, false, cloneDocs, null,
          null, null, addDVFields);
            ...
            while (true) {
              ...
              try {
                System.out.println(String.format(Locale.ENGLISH,
                    ...
                    (System.currentTimeMillis() - startMS) / 1000.0, writer.getDocStats().maxDoc,
...

    {
      ...
      try {
        System.out.println(
            "Searcher: numDocs=" + s.getIndexReader().numDocs() + " maxDoc=" + s.getIndexReader().maxDoc() + ": " + s);
      } finally {
        mgr.release(s);
      }
    }
    ...
    if (!tasksFile.startsWith("server:")) {
      System.out.println("error tasksFile not start with server:");
      System.exit(0);
    }
